...
# define variables
DVD_VOLUME_LABEL="WALLACEANDGROMI"
DVD_RELEASE_DATE="04 Dec 2000"
...
DVD_TITLE=$(rip_dvd_title "DVD rips/$DVD_NAME" 3 1 "en" "A Grand Day Out" "$DVD_RELEASE_DATE" "$DVD_IMAGE" "1. A Grand Day Out" "$DVD_NAME")
if [ $? -ne 0 ]
then
    exit 1
fi
...
DVD_TITLE=$(rip_dvd_title "DVD rips/$DVD_NAME" 2 1 "en" "The Wrong Trousers" "$DVD_RELEASE_DATE" "$DVD_IMAGE" "2. The Wrong Trousers" "$DVD_NAME")
if [ $? -ne 0 ]
then
    exit 1
fi
...
DVD_TITLE=$(rip_dvd_title "DVD rips/$DVD_NAME" 4 1 "en" "A Close Shave" "$DVD_RELEASE_DATE" "$DVD_IMAGE" "3. A Close Shave" "$DVD_NAME")
if [ $? -ne 0 ]
then
    exit 1
fi
